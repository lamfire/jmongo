...
import com.lamfire.jmongo.converters.DefaultConverters;
import com.lamfire.jmongo.converters.TypeConverter;
import com.lamfire.jmongo.mapping.cache.DefaultEntityCache;
import com.lamfire.jmongo.mapping.cache.EntityCache;
import com.lamfire.jmongo.mapping.lazy.DatastoreProvider;
import com.lamfire.jmongo.mapping.lazy.DefaultDatastoreProvider;
...
public class Mapper {
	...
	/** The @{@link Id} field name that is stored with jmongo. */
	...
	/**
	 * Special field used by jmongo to support various possibly loading issues;
	...
	public Object toMongoObject(MappedField mf, MappedClass mc, Object value) {
		...
		if ((mf != null && (mf.hasAnnotation(Reference.class) || mf.getType().isAssignableFrom(Key.class) || mf.getType().isAssignableFrom(DBRef.class) ||
		// Collection/Array/???
				(value instanceof Iterable && mf.isMultipleValues() && (mf.getSubClass().isAssignableFrom(Key.class) || mf.getSubClass().isAssignableFrom(DBRef.class)))))
	...
	private void readMappedField(DBObject dbObject, MappedField mf, Object entity, EntityCache cache) {
		if (mf.hasAnnotation(Property.class) || mf.hasAnnotation(Serialized.class) || mf.isTypeMongoCompatible() || converters.hasSimpleValueConverter(mf))
		...
		if (validateNames) {
			...
			for (int i = 0;;) {
				...
				if (mf == null) {
					...
					if (mf == null)
						throw new ValidationException("The field '" + part + "' could not be found in '" + clazz.getName() + "' while validating - " + prop
			...
			if (validateTypes)
				if ((mf.isSingleValue() && !isCompatibleForOperator(mf.getType(), op, val))
					...
					if (log.isWarnEnabled()) {
						...
						log.warn("The type(s) for the query/update may be inconsistent; using an instance of type '" + val.getClass().getName() + "' for the field '"
								+ mf.getDeclaringClass().getName() + "." + mf.getJavaFieldName() + "' which is declared as '" + mf.getType().getName()
								+ (ste == null ? "'" : "'\r\n --@--" + ste));
	...
	private static StackTraceElement getFirstClientLine(Throwable t) {
		for (StackTraceElement ste : t.getStackTrace())
			if (!ste.getClassName().startsWith("com.google.code.jmongo") && !ste.getClassName().startsWith("sun.reflect") && !ste.getClassName().startsWith("org.junit")
					&& !ste.getClassName().startsWith("org.eclipse") && !ste.getClassName().startsWith("java.lang"))
				return ste;
