...
import com.lamfire.jmongo.mapping.lazy.DatastoreHolder;
...
import com.lamfire.jmongo.query.UpdateException;
...
/**
 * A generic (type-safe) wrapper around jmongo collections
 * @author lamfire
 */
...
public class DatastoreImpl implements Datastore, AdvancedDatastore {
	...
	public void authenticate(String username,char[] password){
		if (username != null){
			if (!this.db.authenticate(username, password)) {
				throw new AuthenticationException("User '" + username + "' cannot be authenticated with the given password for database '" + db.getName() + "'");
			}
		}
	}

	public DatastoreImpl copy(String db) {
		return new DatastoreImpl(mapper, mongo, db);
	}
	...
	public void ensureCaps() {
		for (MappedClass mc : mapper.getMappedClasses())
			if (mc.getEntityAnnotation() != null && mc.getEntityAnnotation().cap().value() > 0) {
				...
				String collName = mapper.getCollectionName(mc.getClazz());
	...
	public <T> List<Key<T>> getKeysByRefs(List<DBRef> refs) {
		...
		for (DBRef ref : refs) {
			Key<T> testKey = mapper.refToKey(ref);
	...
	public DBCollection getCollection(Class clazz) {
		String collName = mapper.getCollectionName(clazz);
	...
	public DBCollection getCollection(String kind) {
	...
	private <T> UpdateResults<T> update(Query<T> query, DBObject u, boolean createIfMissing, boolean multi, WriteConcern wc) {
		...
		if (log.isDebugEnabled())
			log.debug("Executing update(" + dbColl.getName() + ") for query: " + q + ", ops: " + u + ", multi: " + multi + ", upsert: " + createIfMissing);
	...
	public <T> T findAndDelete(Query<T> query) {
		...
		if (log.isDebugEnabled())
			log.debug("Executing findAndModify(" + dbColl.getName() + ") with delete ...");

		DBObject result = dbColl.findAndModify(qi.getQueryObject(), qi.getFieldsObject(), qi.getSortObject(), true, null, false, false);

		if (result != null) {
			T entity = (T) mapper.fromDBObject(qi.getEntityClass(), result, cache);
	        return entity;
		}
	...
	public <T> MapreduceResults<T> mapReduce(MapreduceType type, Query q, Class<T> outputType, MapReduceCommand baseCommand) {
		...
		MapreduceResults mrRes = (MapreduceResults) mapper.fromDBObject(MapreduceResults.class, mpo.getRaw(), createCache());
	...
	public <T> MapreduceResults<T> mapReduce(MapreduceType type, Query query, String map, String reduce, String finalize, Map<String, Object> scopeFields, Class<T> outputType) {
		...
		String outColl = mapper.getCollectionName(outputType);
	...
	public DBDecoderFactory getDecoderFactory() { return decoderFactory != null ? decoderFactory : mongo.getMongoOptions().dbDecoderFactory; }
}
