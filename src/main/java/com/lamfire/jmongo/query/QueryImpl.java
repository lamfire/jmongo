...
public class QueryImpl<T> extends CriteriaContainerImpl implements Query<T>, Criteria {
	...
	private boolean isEnableReplicaSet(){
		try{
			return this.ds.getMongo().getReplicaSetStatus().getName() != null;
		}catch(Exception e){
	...
	public long countAll() {
		...
		if (log.isDebugEnabled())
			log.debug("Executing count(" + dbColl.getName() + ") for query: " + query);
	...
	public List<Object> distinct(String fieldName){
		DBObject query = getQueryObject();
		if (log.isDebugEnabled())
			log.debug("Executing distinct(" + dbColl.getName() + ") for query: " + query);
		return dbColl.distinct(fieldName, query);
	}
	...
	public DBCursor prepareCursor() {
		...
		if (log.isDebugEnabled())
			log.debug("Running query(" + dbColl.getName() + ") : " + query + ", fields:" + fields + ",off:" + offset + ",limit:" + limit);

		DBCursor cursor = dbColl.find(query, fields);
		cursor.setDecoderFactory( this.ds.getDecoderFactory() );
	...
	@Override
	public Query<T> includeFields(String... fields) {
